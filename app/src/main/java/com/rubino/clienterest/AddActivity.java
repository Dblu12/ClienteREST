...
public class AddActivity extends AppCompatActivity {
    ...
    public void accept(View v){
        ...
        Actividad a= new Actividad("",prof.getText().toString(), tipo, hi.getText().toString(), hf.getText().toString(),
                li.getText().toString(), lf.getText().toString(), editText7.getText().toString(), "davidg");

        Log.v("HOLA2",a.toString());

        Cliente api = r.create(Cliente.class);
        Call<Actividad> call = api.createActividad(a);

        call.enqueue(new Callback<Actividad>() {
            @Override
            public void onResponse(Response<Actividad>
                                           response, Retrofit retrofit) {
                Log.v("HOLA",response.raw().toString());
                setResult(RESULT_OK);
                finish();
            }
